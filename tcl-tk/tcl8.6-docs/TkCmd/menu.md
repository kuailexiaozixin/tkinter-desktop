...
**[-activeborderwidth, activeBorderWidth, BorderWidth](options.md#M-activeborderwidth)**
...
The
**menu**
 command creates a new top-level window (given
by the
*pathName*
 argument) and makes it into a menu widget.
That menu widget can either be used as a pop-up window or applied to a

**[toplevel](toplevel.md)**
 (with its
**-menu**
 option) to make it into the menubar for
that toplevel.
Additional
...
Menu entries are displayed with up to three separate fields. The main field is a label in the form of a text string, a bitmap, or an image, controlled by the **-label**, **-bitmap**, and **-image** options for the entry. If the **-accelerator** option is specified for an entry then a second textual field is displayed to the right of the label. The accelerator typically describes a keystroke sequence that may be used in the application to cause the same result as invoking the menu entry. This is a display option, it does not actually set the corresponding binding (which can be achieved using the **[bind](bind.md)** command). The third field is an *indicator*. The indicator is present only for checkbutton or radiobutton entries. It indicates whether the entry is selected or not, and is displayed to the left of the entry's string.
...
**-offvalue**
 option) is stored in the global variable.
...
the menu.  If a
**-command**
 option is specified for a checkbutton
...
**-variable**
 options for the entry).  This action
...
center is displayed in the color given by the
**-selectcolor**
 option
for the entry;
otherwise the indicator's center is displayed in the background color for
the menu.  If a
**-command**
 option is specified for a radiobutton
entry, then its value is evaluated as a Tcl command each time the entry
...
by the
**-menu**
 option).  Cascade entries allow the construction
of cascading menus.
The
**postcascade**
 widget command can be used to post and unpost
...
where
*menu*
 is the path name of the associated menu, and
*x*

and
*y*
 are the root-window coordinates of the upper-right
...
where
*menu*
 is the name of the associated menu.
...
**-tearoff**
 option.  It is not like other menu entries in that
it cannot be created with the
**add**
 widget command and
cannot be deleted with the
**delete**
 widget command.
...
**[toplevel](toplevel.md)**
 command for syntax). On the Macintosh, whenever the
...
by *x* and *y* (in pixel coordinates).
If no entry covers that point, then this form is equivalent to **none**.
...
matching entry is found.  The rules of **[string match](../TclCmd/string.md)**
are used.
...
be achieved using the **[bind](bind.md)** command). This option is not available
...
by some previous invocation of **[image create](image.md)**.
...
**[toplevel](toplevel.md)** manual entry for details.
...
menubutton;  see the **[menubutton](menubutton.md)** manual entry for details.
**Popup Menus**
...
then you call the **[tk_popup](popup.md)** procedure at the appropriate time
