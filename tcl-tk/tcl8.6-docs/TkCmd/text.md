...
**[-exportselection, exportSelection, ExportSelection](options.md#M-exportselection)**
...
**[-setgrid, setGrid, SetGrid](options.md#M-setgrid)**
...
**any**
 and
**display**
. If the submodifier is
abbreviated, then it must be followed by whitespace, but otherwise there need
be no space between the submodifier and the following
*modifier*
.
Typically the
**display**
 submodifier adjusts the meaning of the following

*modifier*
 to make it refer to visual or non-elided units rather than
logical units, but this is explained for each relevant case below. Lastly,
where
*count*
 is used as part of a modifier, it can be positive or
negative, so
“
*base*
 - -3 lines”
...
*mark*
Indicates the character just after the mark whose name is *mark* (see
**MARKS** for details).
*tag***.first**
...
**-exportselection**
 option
for the text widget is true then the
**sel**
 tag will be associated with the
selection:
...
The mark named
**insert**
 has special significance in text widgets. It is
...
widget command. The
**insert**
 mark represents the position of the insertion
...
flag changes state a
**<<Modified>>**
 virtual event is generated. See the
...
**-undo**
 widget option is true) which records every insert and delete
action on a stack.
...
*PathName*
 is the name of the command, which is the same as the text
widget's path name.
*Option*
 and the
*arg*
...
additional possible option **-update** which is a modifier. If given (and
...
for each line). This **-update** option is obsoleted by *pathName***sync**, *pathName***pendingsync** and **<<WidgetViewSync>>**.  The
...
If *index2* does not specify a position later in the text
than *index1* then no characters are deleted.
If *index2* is not specified then the single character at
*index1* is deleted.
...
command depends on the *option* argument that follows the **image**
...
with the given characters and tags.  See the section on *pathName***insert** for an explanation of the handling of the *tagList...*
arguments, and the section on *pathName***delete** for an explanation of the handling of the indices.  If
...
using the rules for regular expressions (see the **[regexp](../TclCmd/regexp.md)** command
and the **[re_syntax](../TclCmd/re_syntax.md)** page for
...
*\n*, which they will otherwise not do (see the **[regexp](../TclCmd/regexp.md)** command for
...
against
“hello there”
will just match twice (i.e. no different to just **-all**), but matching
“Z[a-z]+Z”
against
“ZooZooZoo”
...
The command returns an empty string.
*pathName***sync -command***command*
Schedules *command* to be executed (by the event loop) exactly once as soon
...
string. **[bgerror](../TclCmd/bgerror.md)** is called on *command* failure.
...
the
**[tcl_wordchars](../TclCmd/library.md)**
 variable.  See
**[tclvars](../TclCmd/tclvars.md)**
(n).
...
*pathName*
**search -regexp**
 sub-command attempts to perform
...
Similarly if we add
**-all**
 to this case, it matches at all of
“5.0”,
“4.0”,
“3.0”
and
