...
The
**font**
 command provides several facilities for dealing with
...
*char* argument is supplied, it must be a single character. The font
...
font does not contain the given character.  If *char* may be a hyphen, it
should be preceded by **--** to distinguish it from a misspelled
*option*.
...
form **font***x*, where *x* is an integer.  There may be any number
...
of *text*, not including the extra pixels used by highly exaggerated
...
list of all the metrics and their values.  See **FONT METRICS**
...
*font*
 is specified as an argument above; these same forms are also
permitted when specifying the
**-font**
 option for widgets.
...
name.  See **PLATFORM SPECIFIC FONTS** for a list of the system fonts.
...
described for **-size** in **FONT OPTIONS** below.  Any
additional optional
arguments following the *size* are font *style*s.  Possible values
...
“typewriter”
font), **Times** (a serifed
“newspaper”
font), and **Helvetica** (a sans-serif
“European”
