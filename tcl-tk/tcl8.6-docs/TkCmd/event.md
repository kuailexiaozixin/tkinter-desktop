...
**[winfo id](winfo.md)**) as long as it is for a window in the current application.
...
argument of the **[bind](bind.md)** command except that it must consist
...
“%”
expansions allowed in binding scripts for the
**[bind](bind.md)**
 command.
...
Valid for **KeyPress**, **KeyRelease**, **ButtonPress**,
**ButtonRelease**, and **Motion** events.  Tk supports pointer warping
...
(as an accelerator indicator) with the **-underline** option has
...
**event add**
 command.  Second, a binding must be created for
the virtual event with the
**[bind](bind.md)**
 command.
...
In the **[bind](bind.md)** command, a virtual event can be bound like any other builtin event type as follows:
