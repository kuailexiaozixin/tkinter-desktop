...
The
**focus**
 command is used to manage the Tk input focus.
At any given time, one window on each display is designated as
the
*focus window*
;  any key press or key release events for the
...
If the focus window for *window*'s display is not in this
