...
Note that for management of the **CLIPBOARD** selection (see below), the **[clipboard](clipboard.md)** command may also be used.
...
formats, such as **STRING**, **UTF8_STRING**, **ATOM**,
**INTEGER**, etc. (this format is different
...
**[ttk::spinbox](ttk_spinbox.md)**
 widgets have the option
**-exportselection**
.  If a widget has this option set to boolean
**true**
, then (in an unsafe interpreter) a selection made in the widget is automatically written to the
**PRIMARY**
 selection.
...
On X11, the
**PRIMARY**
 selection is a system-wide feature of the X server, allowing communication between different processes that are X11 clients.
...
A safe interpreter cannot read from the
**PRIMARY**
 selection because its
**selection**
 command is hidden.  For this reason the
**PRIMARY**
...
**SECONDARY**
 selection. Hardly anything uses it, but here is how to read
it using Tk:
