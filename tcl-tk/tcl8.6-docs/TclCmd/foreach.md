...
The
**foreach**
 command implements a loop where the loop
...
and one list,
*list*
, that is a list of values to assign to
*varname*
.
The
*body*
 argument is a Tcl script.
For each element of
*list*
 (in order
from first to last),
...
The **[break](break.md)** and **[continue](continue.md)** statements may be invoked inside *body*, with the same effect as in the **[for](for.md)** command. **Foreach** returns an empty string.
