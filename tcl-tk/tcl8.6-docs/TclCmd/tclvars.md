...
Additional variables relating to package management exist. More details are listed in the **VARIABLES** section of the **[library](library.md)** manual page.
...
passed to children by commands like **[exec](exec.md)**.
...
call of **[cd](cd.md)** without arguments or with just
“~”
...
dates in the **[clock](clock.md)** command. On many platforms, the TZ environment
variable is set up by the operating system.
**env(LC_ALL)**, **env(LC_MESSAGES)**, **env(LANG)**
These environment variables are used by the **[msgcat](msgcat.md)** package to
determine what locale to format messages using.
**env(TCL_INTERP_DEBUG_FRAME)**
If existing, it has the same effect as running **[interp debug](interp.md)**
**{} -frame 1**
...
to divide zero by zero in the **[expr](expr.md)** command).
...
To set the **-errorcode** return option, applications should use library procedures such as **Tcl_SetObjErrorCode**, **Tcl_SetReturnOptions**, and **Tcl_PosixError**, or they may invoke the **-errorcode** option of the **[return](return.md)** command. If none of these methods for setting the error code has been used, the Tcl interpreter will reset the variable to **NONE** after the next error.
...
The value of this variable is returned by the **[info library](info.md)** command.
See the **[library](library.md)** manual entry for details of the facilities
...
**$***app***_library** (where *app* is the application's name)
...
The value of this variable is returned by the **[info patchlevel](info.md)**
command.
**tcl_pkgPath**
...
searched for packages during **[package require](package.md)** commands.  Note:
...
created, this is always set to the string **[Tcl](Tcl.md)**.
**machine**
...
**pathSeparator**
The character that should be used to **[split](split.md)** PATH-like environment
...
to 0.  *Applications should not change this value;* it is
...
for *tcl_precision* in that an inexact number like *1.4*
will convert as *1.4* rather than *1.3999999999999999*
...
a **tcl_precision** value of 17 digits is
“perfect”
...
of this file and **[source](source.md)** it if it exists.  For example, for **wish**
...
commands such as **[set](set.md)** and **[incr](incr.md)**
...
The value of this variable is returned by the **[info tclversion](info.md)**
command.
...
and
**wish**
 executables; the Tcl library does not define them
...
, e.g., because of some application-specific
...
The **argv0** global variable can be used (in conjunction with the **[info script](info.md)** command) to determine whether the current script is being executed as the main script or loaded as a library. This is useful because it allows a single script to be used as both a library and a demonstration of that library:
