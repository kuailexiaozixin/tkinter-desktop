...
to the ANSI C
**sscanf**
 procedure and returns a count of the number of
...
to be parsed and
*format*
 indicates how to parse it, using
**%**
...
*format*
 is a blank or tab then it
matches any number of white space characters in
*string*
 (including
zero).
Otherwise, if it is not a
**%**
 character then it
must match the next character of
*string*
.
When a
**%**
...
by the number, where 1 corresponds to the first
*varName*
.  If
there are any positional specifiers in
*format*
 then all of the
specifiers must be positional.  Every
*varName*
 on the argument
...
The syntactically valid values for the size modifier are
**h**
,
**L**
,
...
.  The
**h**
 size modifier value is equivalent
...
command.  The
**L**
 size modifier is equivalent to the
**l**
 size
modifier. Either one indicates the integer range to be stored is
limited to the same range produced by the
**[wide()](mathfunc.md)**
 function of
the
**[expr](expr.md)**
...
The behavior of the
**scan**
 command is the same as the behavior of
the ANSI C
**sscanf**
