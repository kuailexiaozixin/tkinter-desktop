...
reference page.)
If there is a match, then while copying
*string*
 to
*varName*
...
”
and backslashes.
The use of backslashes in
*subSpec*
 tends to interact badly
with the Tcl parser's use of backslashes, so it is generally
safest to enclose
*subSpec*
 in braces if it includes
backslashes.
...
the **(?x)** embedded option (see the **[re_syntax](re_syntax.md)** manual page).
**-line**
...
**(?n)** embedded option (see the **[re_syntax](re_syntax.md)** manual page).
**-linestop**
...
embedded option (see the **[re_syntax](re_syntax.md)** manual page).
**-lineanchor**
Changes the behavior of
“^”
and
...
specifying the **(?w)** embedded option (see the **[re_syntax](re_syntax.md)**
manual page).
**-nocase**
...
as the *index* argument to **[string index](string.md)**.
...
If *varName* is supplied, the command returns a count of the number of matching ranges that were found and replaced, otherwise the string after replacement is returned. See the manual entry for **[regexp](regexp.md)** for details on the interpretation of regular expressions.
...
Convert all non-ASCII and Tcl-significant characters into \u escape sequences by using **regsub** and **[subst](subst.md)** in combination:
