...
of them matches
*pattern*
.  If so, the command returns the index
...
or
**-inline**
 are specified) the empty string.  The
*option*
 arguments
indicates how the elements of the list are to be matched against

*pattern*
 and must have one of the values below:
...
is
**-glob**
.  If more than one matching style is specified, the
...
element using the same rules as the **[string match](string.md)** command.
**-regexp**
*Pattern* is treated as a regular expression and matched against
each list element using the rules described in the **[re_syntax](re_syntax.md)**
reference page.
**-sorted**
...
for the command **[string index](string.md)**, supporting simple index
...
and
**-sorted**
 options.  If more than one is specified, the last
...
comparisons (see **[lsort](lsort.md)** for a fuller description). Note that this
...
used with the **[lindex](lindex.md)** or **[lset](lset.md)** commands) within each element
...
path (suitable for use with **[lindex](lindex.md)** or **[lset](lset.md)**) within the
