...
*locale*
?
**::msgcat::mcpackageconfig subcommand**
*option*
 ?
*value*
?
...
are given, the **[format](format.md)** command is used to substitute the
...
the language specifications returned by **::msgcat::mcloadedlocales get**
...
- The file contains a series of calls to **mcflset** and **mcflmset**, setting the necessary translation strings for the language, likely enclosed in a **[namespace eval](namespace.md)** so that all source strings are tied to the namespace of the package. For example, a short **es.msg** might contain:
...
to
**[format](format.md)**
 might have positionally dependent parameters that
...
Similarly, positional parameters can be used with **[scan](scan.md)** to extract values from internationalized strings. Note that it is not necessary to pass the output of **::msgcat::mc** to **[format](format.md)** directly; by passing the values to substitute in as arguments, the formatting substitution is done directly.
...
A package using
**msgcat**
 may choose to use its own package private
...
A generic unknown handler is used if set to the empty string. This consists of returning the key if no arguments are given. With given arguments, the **[format](format.md)** command is used to process the arguments.
...
If a called routine fails with an error, the **[bgerror](bgerror.md)** routine for the interpreter is invoked after command completion. Only exception is the callback **unknowncmd**, where an error causes the invoking **mc**-command to fail with that error.
...
The **[clock](clock.md)** command implementation uses **msgcat** with a package locale to implement the command line parameter **-locale**. Here are some sketches of the implementation:
...
**[clock](clock.md)**
 sets the package locale to
**fr**
 and looks for the day name as
follows:
...
Within
**[clock](clock.md)**
, some message-catalog items are heavy in computation and
