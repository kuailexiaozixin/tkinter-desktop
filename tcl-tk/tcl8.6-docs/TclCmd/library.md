...
auto_execok, auto_import, auto_load, auto_mkindex, auto_qualify, auto_reset, parray, tcl_findLibrary, tcl_endOfWord, tcl_nonwordchars, tcl_startOfNextWord, tcl_startOfPreviousWord, tcl_wordBreakAfter, tcl_wordBreakBefore, tcl_wordchars — standard library of Tcl procedures
...
If the library procedure **Tcl_Init** is invoked from an application's **Tcl_AppInit** procedure, this happens automatically. The code in **init.tcl** will define the **[unknown](unknown.md)** procedure and arrange for the other procedures to be loaded on-demand using the auto-load mechanism defined below.
...
passed to **[exec](exec.md)** to execute the executable file or shell builtin
...
directories and file extensions as used by **[exec](exec.md)**. **Auto_execok**
...
**auto_import***pattern*
**Auto_import** is invoked during **[namespace import](namespace.md)** to see if
...
matching rules of **[namespace import](namespace.md)**.
...
global variable **[auto_path](tclvars.md)** if it exists.  If there is no
**[auto_path](tclvars.md)** variable, then the **TCLLIBPATH** environment variable is
...
It is not normally necessary to call this command directly; the default **[unknown](unknown.md)** handler will do so.
...
*pattern* arguments (matching is done with the **[glob](glob.md)**
...
The *patch* argument is not used.
...
matching rules of **[string match](string.md)**) and their values if *pattern* is
given.
*ArrayName* must be an array accessible to the caller of **parray**.
It may be either local or global.
For example, to print the contents of the **[tcl_platform](tclvars.md)** array, do:
...
**auto_noexec**
If set to any value, then **[unknown](unknown.md)** will not attempt to auto-exec
any commands.
**auto_noload**
If set to any value, then **[unknown](unknown.md)** will not attempt to auto-load
any commands.
**auto_path**
If set, then it must contain a valid Tcl list giving directories to
search during auto-load operations (including for package index
files when using the default **[package unknown](package.md)** handler).
This variable is initialized during startup to contain, in order:
the directories listed in the **TCLLIBPATH** environment variable,
the directory named by the **[tcl_library](tclvars.md)** global variable,
the parent directory of **[tcl_library](tclvars.md)**,
the directories listed in the **[tcl_pkgPath](tclvars.md)** variable.
Additional locations to look for files and package indices should
normally be added to this variable using **[lappend](lappend.md)**.
...
Note that if the script uses **[cd](cd.md)**, it is advisable to ensure that entries on the **[auto_path](tclvars.md)** are **[file normalize](file.md)**d.

**env(TCL_LIBRARY)**
If set, then it specifies the location of the directory containing
library scripts (the value of this variable will be
assigned to the **[tcl_library](tclvars.md)** variable and therefore returned by
the command **[info library](info.md)**).  If this variable is not set then
...
This variable is only used when initializing the **[auto_path](tclvars.md)** variable.
A key consequence of this variable is that it gives a way to let the user of a script specify the list of places where that script may use **[package require](package.md)** to read packages from. It is not normally usefully settable within a Tcl script itself *except* to influence where other interpreters load from (whether made with **[interp create](interp.md)** or launched as their own threads or subprocesses).
