...
Modules (see
**MODULE DEFINITION**
 for the definition of a Tcl Module).
...
and
“**tcl***X***/***X***.***y*”,
for major version *X* of the
Tcl interpreter and minor version *y* less than or equal to the
...
files required by it. This file has to be
**[source](source.md)**
able. In other
...
The **[load](load.md)** command is not directly used. This restriction is not an actual limitation, as some may believe. Ever since 8.4 the Tcl **[source](source.md)** command reads only until the first ^Z character. This allows us to combine an arbitrary Tcl script with arbitrary binary data into one file, where the script processes the attached data in any it chooses to fully import and activate the package.
...
Tcl Modules are searched for in all directories listed in the result of the command **::tcl::tm::path list**. This is called the *Module path*. Neither the **[auto_path](tclvars.md)** nor the **[tcl_pkgPath](tclvars.md)** variables are used. All directories on the module path have to obey one restriction:
...
**tclsh**
 as follows, where
*X*
 is the major version of the Tcl
interpreter and
*y*
 is less than or equal to the minor version of
the Tcl interpreter.
...
Where **EXEC** is **file normalize [info nameofexecutable]/../lib**
or **file normalize [::tcl::pkgconfig get libdir,runtime]**
...
“*Tcl Modules*”
(online at https://tip.tcl-lang.org/189.html)**
,
**Tcl Improvement Proposal #190
