...
subcommands (termed an
*ensemble*
, as it is a command such as that
created by
**[namespace ensemble](namespace.md)**
**create**
, though the implementation
of handlers for reflected channel
*is not*
 tied to
**namespace
ensemble**
s in any way; see
**EXAMPLE**
 below for how to build an

**[oo::class](class.md)**
 that supports the API). Note that
*cmdPrefix*
 is whatever was
...
Any error thrown by the method will abort the creation of the channel and no channel will be created. The thrown error will appear as error thrown by **[chan create](chan.md)**. Any exception other than an **[error](error.md)** (e.g., **[break](break.md)**, etc.) is treated as (and converted to) an error.

**Note:** If the creation of the channel was aborted due to failures here, then the **finalize** subcommand will not be called.

The *mode* argument tells the handler whether the channel was opened for reading, writing, or both. It is a list containing any of the strings **[read](read.md)** or **write**. The list may be empty, but will usually contain at least one element.
...
If the subcommand throws an error the command which caused its invocation (usually **[chan close](chan.md)**) will appear to have thrown this error. Any exception beyond **[error](error.md)** (e.g., **[break](break.md)**, etc.) is treated as (and converted to) an error.
...
*eventspec*. This argument is a list containing any of **[read](read.md)**
...
**Warning:** Any return value of the subcommand is ignored. This includes all errors thrown by the subcommand, **[break](break.md)**, **[continue](continue.md)**, and custom return codes.

This subcommand interacts with **[chan postevent](chan.md)**. Trying to post an event which was not listed in the last call to **watch** will cause **[chan postevent](chan.md)** to throw an error.
...
The return value of this subcommand is taken as the requested data *bytes*. If the returned data contains more bytes than requested, an error will be signaled and later thrown by the command which performed the read (usually **[gets](gets.md)** or **[read](read.md)**). However, returning fewer bytes than requested is acceptable.

Note that returning nothing (0 bytes) is a signal to the higher layers that **[EOF](eof.md)** has been reached on the channel. To signal that the channel is out of data right now, but has not yet reached **[EOF](eof.md)**, it is necessary to throw the error "EAGAIN", i.e. to either
...
If the subcommand throws any other error, the command which caused its invocation (usually **[gets](gets.md)**, or **[read](read.md)**) will appear to have thrown this error. Any exception beyond **[error](error.md)**, (e.g., **[break](break.md)**, etc.) is treated as and converted to an error.
...
If the subcommand throws any other error the command which caused its invocation (usually **[puts](puts.md)**) will appear to have thrown this error. Any exception beyond **[error](error.md)** (e.g., **[break](break.md)**, etc.) is treated as and converted to an error.

*cmdPrefix***seek***channelId offset base*
This *optional* subcommand is responsible for the handling of
**[chan seek](chan.md)** and **[chan tell](chan.md)** requests on the channel
*channelId*. If it is not supported then seeking will not be possible for
the channel.
The *base* argument is the same as the equivalent argument of the builtin **[chan seek](chan.md)**, namely:
...
The return value of the subcommand is taken as the (new) location of the channel, counted from the start. This has to be an integer number greater than or equal to zero. If the subcommand throws an error the command which caused its invocation (usually **[chan seek](chan.md)**, or **[chan tell](chan.md)**) will appear to have thrown this error. Any exception beyond **[error](error.md)** (e.g., **[break](break.md)**, etc.) is treated as and converted to an error.

The offset/base combination of 0/**current** signals a **[chan tell](chan.md)** request, i.e., seek nothing relative to the current location, making the new location identical to the current one, which is then returned.
...
If the subcommand throws an error the command which performed the (re)configuration or query (usually **[fconfigure](fconfigure.md)** or **[chan configure](chan.md)**) will appear to have thrown this error. Any exception beyond **[error](error.md)** (e.g., **[break](break.md)**, etc.) is treated as and converted to an error.
...
If the subcommand throws an error, the command which performed the (re)configuration or query (usually **[fconfigure](fconfigure.md)** or **[chan configure](chan.md)**) will appear to have thrown this error. Any exception beyond *error* (e.g., **[break](break.md)**, etc.) is treated as and converted to an error.
...
If the subcommand throws an error the command which caused its invocation (usually **[fconfigure](fconfigure.md)** or **[chan configure](chan.md)**) will appear to have thrown this error. Any exception beyond **[error](error.md)** (e.g., **[break](break.md)**, etc.) is treated as and converted to an error.
