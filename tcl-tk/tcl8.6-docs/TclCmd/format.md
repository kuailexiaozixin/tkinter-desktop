...
*FormatString*
 indicates how to format the result, using
...
**%**
 then it is not copied to the result string.
...
then each one controls the conversion of one additional
*arg*
.
The
**format**
...
*arg*
s to meet the needs
of all of the conversion specifiers in
*formatString*
.
...
If the minimum field width is specified as
*****
 rather than
...
, and
**f**
 conversions it specifies the number
of digits to appear to the right of the decimal point.
For
**g**
 and
**G**
 conversions it specifies the total number
...
If the precision is specified with
*****
 rather than a number
then the next argument to the
**format**
 command determines the precision;
...
If it is
**ll**
 it specifies that an integer value is taken
without truncation for conversion to a formatted substring.
If it is
**h**
 it specifies that an integer value is
truncated to a 16-bit range before converting.  This option is rarely useful.
If it is
**l**
 it specifies that the integer value is
truncated to the same range as that produced by the
**[wide()](mathfunc.md)**

function of the
**[expr](expr.md)**
 command (at least a 64-bit range).
If neither
**h**
 nor
**l**
 are present, the integer value is
truncated to the same range as that produced by the
**[int()](mathfunc.md)**

function of the
**[expr](expr.md)**
 command (at least a 32-bit range, but
...
form *x.yyy***e±***zz*, where the number of *y*'s is determined
...
- The size modifiers are ignored when formatting floating-point values. The **ll** modifier has no **sprintf** counterpart. The **b** specifier has no **sprintf** counterpart.
...
Convert the output of **[time](time.md)** into seconds to an accuracy of hundredths of a second:
...
Use XPG3 format codes to allow reordering of fields (a technique that is often used in localized message catalogs; see **[msgcat](msgcat.md)**) without reordering the data values passed to **format**:
