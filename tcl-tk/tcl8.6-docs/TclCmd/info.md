...
the command procedures defined using the **[proc](proc.md)** command.
If *pattern* is specified,
only those names matching *pattern* are returned.
Matching is determined using the same rules as for **[string match](string.md)**.
...
**info coroutine**
Returns the name of the currently executing **[coroutine](coroutine.md)**, or the empty
...
This form is an even-sized list alternating tokens and parameters. Tokens are currently either **CALL**, **UP**, or **INNER**, but other values may be introduced in the future. **CALL** indicates a procedure call, and its parameter is the corresponding **info level** **0**. **UP** indicates a shift in variable frames generated by **[uplevel](uplevel.md)** or similar, and applies to the previous **CALL** item. Its parameter is the level offset. **INNER** identifies the “inner context”, which is the innermost atomic command or bytecode instruction that raised the error, along with its arguments when available. While **CALL** and **UP** allow to follow complex call paths, **INNER** homes in on the offending operation in the innermost procedure call, even going to sub-expression granularity.

This information is also present in the **-errorstack** entry of the options dictionary returned by 3-argument **[catch](catch.md)**; **info errorstack** is a convenient way of retrieving it for uncaught errors at top-level in an interactive **tclsh**.
...
This is similar to how **info level** works, except that this subcommand reports all frames, like **[source](source.md)**d scripts, **[eval](eval.md)**s, **[uplevel](uplevel.md)**s, etc.
...
for the command. The recognized values are **[source](source.md)**, **[proc](proc.md)**,
**[eval](eval.md)**, and **precompiled**.

**source**
means that the command is found in a script loaded by the **[source](source.md)**
command.
**proc**
means that the command is found in dynamically created procedure body.
**eval**
means that the command is executed by **[eval](eval.md)** or **[uplevel](uplevel.md)**.
**precompiled**
...
**precompiled**. For type **[source](source.md)** this information is counted
...
**file**
This entry is present only for type **[source](source.md)**. It provides the
...
canonically-constructed list (e.g., as produced by the **[list](list.md)** command)
executed by **[eval](eval.md)** it is the substituted form as they have no other
...
A thing of note is that for procedures statically defined in files the locations of commands in their bodies will be reported with type **[source](source.md)** and absolute line numbers, and not as type **[proc](proc.md)**. The same is true for procedures nested in statically defined procedures, and literal eval scripts in files or statically defined procedures.

In contrast, procedure definitions and **[eval](eval.md)** within a dynamically **[eval](eval.md)**uated environment count line numbers relative to the start of their script, even if they would be able to count relative to the start of the outer dynamic script. That type of number usually makes more sense.

A different way of describing this behaviour is that file based locations are tracked as deeply as possible, and where this is not possible the lines are counted based on the smallest possible **[eval](eval.md)** or procedure body, as that scope is usually easier to find than any dynamic outer scope.

The syntactic form **{*}** is handled like **[eval](eval.md)**. I.e. if it is given a literal list argument the system tracks the line number within the list words as well, and otherwise all line numbers are counted relative to the start of each word (smallest scope)
...
rules as for **[string match](string.md)**.
...
See the **[uplevel](uplevel.md)** command for more information on what stack
levels mean.
**info library**
Returns the name of the library directory in which standard Tcl
scripts are stored.
This is actually the value of the **[tcl_library](tclvars.md)**
variable and may be changed by setting **[tcl_library](tclvars.md)**.
**info loaded**?*interp*?
Returns a list describing all of the packages that have been loaded into
*interp* with the **[load](load.md)** command.
...
Variables defined with the **[global](global.md)**, **[upvar](upvar.md)**  and
**[variable](variable.md)** commands will not be returned.
If *pattern* is specified, only those names matching *pattern*
are returned.  Matching is determined using the same rules as for
**[string match](string.md)**.
...
**info patchlevel**
Returns the value of the global variable **tcl_patchLevel**, which holds
...
of the **[source](source.md)** command), then this command returns the name
...
string is returned.
**info tclversion**
Returns the value of the global variable **[tcl_version](tclvars.md)**, which holds the
...
“exist”
if it has not
been set (e.g. a variable declared but not set by **[variable](variable.md)**).
...
ordinary method, **filter** for an applied filter, and **[unknown](unknown.md)** for a
...
“**[unknown](unknown.md)**”
for the **[unknown](unknown.md)** type), a word giving the fully qualified name of the
class that defined the method, and a word describing the type of method
implementation (see **info class methodtype**).
Note that there is no inspection of whether the method implementations actually use **[next](next.md)** to transfer control along the call chain.
...
passing to another call to **[proc](proc.md)** or a method definition, and the second
...
suitable for passing to another call to **[proc](proc.md)** or a method definition, and
...
instances to those that match it according to the rules of **[string match](string.md)**.
**info class methods***class* ?*options...*?
This subcommand returns a list of all public (i.e. exported) methods of the
class called *class*. Any of the following *option*s may be
...
*method* for the **method** type, and
“**[unknown](unknown.md)**”
for the **[unknown](unknown.md)** type), a word giving what defined the method (the fully
...
form suitable for passing to another call to **[proc](proc.md)** or a method definition,
...
This subcommand returns a list of all public (i.e. exported) methods of the
object called *object*. Any of the following *option*s may be
specified, controlling exactly which method names are returned:
...
given, it is a filter (in the syntax of a **[string match](string.md)** glob pattern)
