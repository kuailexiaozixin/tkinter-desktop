...
The
**zlib**
 command provides access to the compression and check-summing
...
will be in the same as is returned by **[clock seconds](clock.md)** or **[file mtime](file.md)**.
**type**
...
*channel*.
The transformation can be removed again with **[chan pop](chan.md)**.
...
Both compressing and decompressing channel transformations add extra configuration options that may be accessed through **[chan configure](chan.md)**. The options are:
...
Streaming compression instance commands are produced by the
**zlib stream**

command. They are used by calling their
**put**
 subcommand one or more times
to load data in, and their
**get**
 subcommand one or more times to extract
...
buffers while applying the transformation. The following *option*s are
