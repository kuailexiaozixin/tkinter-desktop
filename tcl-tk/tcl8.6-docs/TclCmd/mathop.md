...
The commands in the
**::tcl::mathop**
 namespace implement the same set of
operations as supported by the
**[expr](expr.md)**
 command. All are exported from the
...
*not*
 alter the way that the
**[expr](expr.md)**
 command behaves, and nor does
...
the sense of the **[expr](expr.md)** == operator (*i.e.*, numeric comparison if
...
the **[expr](expr.md)** != operator (*i.e.*, numeric comparison if possible, exact
