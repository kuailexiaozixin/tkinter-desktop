# Windows_NT_Security_.2d.2d_Impersonation

> 来源 https://mhammond.github.io/pywin32/Windows_NT_Security_.2d.2d_Impersonation.html
...
```
 ...
 The api call is very similar in both cases except in python the handle is returned separately to the caller. The interesting options in this case are logonType and logonProvider. To give values for these, you need to use the constants present in win32con (you can use the browser in pythonwin->tools to list the constants in win32con). Unless you have unusual server requirements, for logonType, win32con.LOGON32_LOGON_INTERACTIVE should be fine. With regards to logonProvider, generally use win32con.LOGON32_PROVIDER_DEFAULT -- it's for specifying the type of logon NT 3.5, 4.0, win2000. Generally, default is fine.
