...
The
**next**
 command is used to call implementations of a method by a class,
...
Filters are invoked when processing an invocation of the **unknown** method because of a failure to locate a method implementation, but *not* when invoking either constructors or destructors. (Note however that the **[destroy](../TkCmd/destroy.md)** method is a conventional method, and filters are invoked as normal when it is called.)
