...
**return**
 command is used without options
in the body of a procedure to immediately return control to the caller
of the procedure.  If a
*result*
 argument is provided, its value
becomes the result of the procedure passed back to the caller.
If
*result*
 is not specified then an empty string will be returned
to the caller as the result of the procedure.

The **return** command serves a similar function within script files that are evaluated by the **[source](source.md)** command. When **[source](source.md)** evaluates the contents of a file as a script, an invocation of the **return** command will cause script evaluation to immediately cease, and the value *result* (or an empty string) will be returned as the result of the **[source](source.md)** command.

### EXCEPTIONAL RETURN CODES

In addition to the result of a procedure, the return
code of a procedure may also be set by
**return**
...
However, the
**-code**
 option may be used to generate an
...
of the procedure is 0 (**[TCL_OK](catch.md)**).
**error** (or **1**)
Error return: the return code of the procedure is 1 (**[TCL_ERROR](catch.md)**).
The procedure command behaves in its calling context as if it
were the command **[error](error.md)***result*.  See below for additional
options.
**return** (or **2**)
The return code of the procedure is 2 (**[TCL_RETURN](catch.md)**).  The
procedure command behaves in its calling context as if it
were the command **return** (with no arguments).
**break** (or **3**)
The return code of the procedure is 3 (**[TCL_BREAK](catch.md)**).  The
procedure command behaves in its calling context as if it
were the command **[break](break.md)**.
**continue** (or **4**)
The return code of the procedure is 4 (**[TCL_CONTINUE](catch.md)**).  The
procedure command behaves in its calling context as if it
were the command **[continue](continue.md)**.
*value*
...
The **return -code** command acts similarly within script files that are evaluated by the **[source](source.md)** command. During the evaluation of the contents of a file as a script by **[source](source.md)**, an invocation of the **return -code***code* command will cause the return code of **[source](source.md)** to be *code*.
...
of the **-code** option is **[TCL_ERROR](catch.md)**.  Then the *list* value
...
option will also be stored in the global variable **[errorCode](tclvars.md)**.
**-errorinfo***info*
The **-errorinfo** option receives special treatment only when the value
of the **-code** option is **[TCL_ERROR](catch.md)**.  Then *info* is the initial
...
also be stored in the global variable **[errorInfo](tclvars.md)**.
...
by the **[catch](catch.md)** command (or from the copy of that information
stored in the global variable **[errorInfo](tclvars.md)**).
**-errorstack***list*
The **-errorstack** option receives special treatment only when the value
of the **-code** option is **[TCL_ERROR](catch.md)**.  Then *list* is the initial
error stack, recording actual argument values passed to each proc level.
The error stack will
also be reachable through **[info errorstack](info.md)**.
...
by the **[catch](catch.md)** command (or from the copy of that information from
**[info errorstack](info.md)**).
**-level***level*
...
returns a return code of
**[TCL_OK](catch.md)**
, evaluation will continue to the next
command in the script.  Any exceptional return code (non-
**[TCL_OK](catch.md)**
)
...
Some of Tcl's built-in commands evaluate scripts as part of their functioning. These commands can make use of exceptional return codes to enable special features. For example, the built-in Tcl commands that provide loops — such as **[while](while.md)**, **[for](for.md)**, and **[foreach](foreach.md)** — evaluate a script that is the body of the loop. If evaluation of the loop body returns the return code of **[TCL_BREAK](catch.md)** or **[TCL_CONTINUE](catch.md)**, the loop command can react in such a way as to give the **[break](break.md)** and **[continue](continue.md)** commands their documented interpretation in loops.

Procedure invocation also involves evaluation of a script, the body of the procedure. Procedure invocation provides special treatment when evaluation of the procedure body returns the return code **[TCL_RETURN](catch.md)**. In that circumstance, the **-level** entry in the return options dictionary is decremented. If after decrementing, the value of the **-level** entry is 0, then the value of the **-code** entry becomes the return code of the procedure. If after decrementing, the value of the **-level** entry is greater than zero, then the return code of the procedure is **[TCL_RETURN](catch.md)**. If the procedure invocation occurred during the evaluation of the body of another procedure, the process will repeat itself up the call stack, decrementing the value of the **-level** entry at each level, so that the *code* will be the return code of the current command *level* levels up the call stack. The **[source](source.md)** command performs the same handling of the **[TCL_RETURN](catch.md)** return code, which explains the similarity of **return** invocation during a **[source](source.md)** to **return** invocation within a procedure.

The return code of the **return** command itself triggers this special handling by procedure invocation. If **return** is provided the option **-level 0**, then the return code of the **return** command itself will be the value *code* of the **-code** option (or **[TCL_OK](catch.md)** by default). Any other value for the **-level** option (including the default value of 1) will cause the return code of the **return** command itself to be **[TCL_RETURN](catch.md)**, triggering a return from the enclosing procedure.

### EXAMPLES

First, a simple example of using
**return**
 to return from a
...
Next, a procedure replacement for **[break](break.md)**.
...
With the **-level 0** option, **return** itself can serve as a replacement for **[break](break.md)**, with the help of **[interp alias](interp.md)**.

```
interp alias {} Break {} return -level 0 -code break
```

An example of using **[catch](catch.md)** and **return -options** to re-raise a caught error:
