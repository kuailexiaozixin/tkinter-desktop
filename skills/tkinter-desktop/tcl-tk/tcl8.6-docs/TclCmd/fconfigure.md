...
*ChannelId* identifies the channel for which to set or query an option and must refer to an open channel such as a Tcl standard channel (**stdin**, **stdout**, or **stderr**), the return value from an invocation of **[open](open.md)** or **[socket](socket.md)**, or the result of a channel creation command provided by a Tcl extension.
...
nonblocking mode it will affect the operation of the **[gets](gets.md)**,
**[read](read.md)**, **[puts](puts.md)**, **[flush](flush.md)**, and **[close](close.md)** commands by
...
invoking the **[vwait](vwait.md)** command).
**-buffering** *newValue*
If *newValue* is **full** then the I/O system will buffer output
until its internal buffer is full or until the **[flush](flush.md)** command is
...
or output. *Newvalue* must be between one and one million, allowing
...
normally used in network connections.  On input (i.e., with **[gets](gets.md)**
and **[read](read.md)**) the Tcl I/O system automatically translates the external
end-of-line representation into newline characters.  Upon output (i.e.,
with **[puts](puts.md)**), the I/O system translates newlines to the external
...
**lf** mode, except that in addition **[binary](binary.md)** mode also sets the
end-of-file character to the empty string (which disables it) and sets the
encoding to **[binary](binary.md)** (which disables encoding filtering).  See the
description of **-eofchar** and **-encoding** for more information.
Internally, i.e. when it comes to the actual behaviour of the translator this value **is** identical to **lf** and is therefore reported as such when queried. Even if **[binary](binary.md)** was used to set the translation.
...
If, for example, a Tcl application is started by the
**inet**
