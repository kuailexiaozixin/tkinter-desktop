...
*basic*
 REs
(“BRE”s).
EREs are roughly those of the traditional
*egrep*
, while BREs are
roughly those of the traditional
*ed*
...
**(***re***)**
matches a match for *re* (*re* is any regular expression) with
...
taken as an ordinary character, e.g. **\\** matches a backslash
character
**\***c*
...
A
*constraint*
 matches an empty string when specific conditions
...
matching is newline-sensitive or not, as described in **MATCHING**,
below).
**$**
...
**[.**
 and
**.]**
 stands for the sequence of characters of that
...
**\x***hh*
(where *hh* is one or two hexadecimal digits) the character
whose hexadecimal value is **0x***hh*.
**\0**
...
and the decimal value *mnn* is not greater than the number of
closing capturing parentheses seen so far) a *back reference*, see
below
...
**{**
 and
**}**
 by themselves ordinary characters. The
