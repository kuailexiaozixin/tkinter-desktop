...
The
**namespace**
 command lets you create, access, and destroy
...
*subcommand*
 are listed below.
Note that you can abbreviate the
*subcommand*
s.
...
which start with a double colon (**::**).
...
a pattern that starts with double colon (**::**) is used directly,
...
in a **namespace inscope** command.
...
See the section **SCOPED SCRIPTS** for some examples
...
in the same fashion as the **[eval](eval.md)** command,
...
Each *pattern* is a simple or qualified name such as
**x**, **foo::x** or **a::b::p***.
Qualified names contain double colons (**::**) and qualify a name
...
In effect, this undoes the action of a **namespace import** command.
...
**namespace import** returns the list of commands in
...
This format is suitable for composition with **namespace forget**
(see **EXAMPLES** below).
...
command except that the *namespace* must already exist,
and **namespace inscope** appends additional *arg*s
...
resolved as described in section **NAME RESOLUTION**.
The command
**namespace upvar $ns a b** has the same behaviour as
**upvar 0 ${ns}::a b**, with the sole exception of the resolution rules
...
The *script* argument, if given, should be a well
...
created but not defined, such as with the **[variable](variable.md)** command
or through a **[trace](trace.md)** on the variable, this command will return the
...
Namespace variables resemble global variables in Tcl. They exist outside of the procedures in a namespace but can be accessed in a procedure via the **[variable](variable.md)** command, as shown in the example above.
...
Note that the **test** procedure is added to the **Counter** namespace, and later removed via the **[rename](rename.md)** command.
...
You can access a namespace variable from a procedure in the same namespace by using the **[variable](variable.md)** command. Much like the **[global](global.md)** command, this creates a local link to the namespace variable. If necessary, it also creates the variable in the current namespace and initializes it. Note that the **[global](global.md)** command only creates links to variables in the global namespace. It is not necessary to use a **[variable](variable.md)** command if you always refer to the namespace variable using an appropriate qualified name.
...
like BLT are contained in a namespace called
**Blt**
.
...
The
**namespace code**
 command is the means by which a script may be
...
code indicates how to direct a variable
**[trace](trace.md)**
 callback into the current
namespace:
...
The
**namespace ensemble**
 is used to create and manipulate ensemble
...
ensemble command.  See the section **ENSEMBLE OPTIONS** below for a
...
similar to an alias created with **[interp alias](interp.md)**; the words are not
...
*current* namespace and changed to be exactly that (that is, it is
...
**-subcommands** option is empty, the ensemble subcommand names
...
is defined in the **-map** option, or to the command with the same
...
dictionary listed in the **-map** option or the exported commands
