...
must be the same as the filename provided to
**[load](load.md)**
 for
loading the library.
The
*prefix*
 argument is the prefix (as
determined by or passed to
**[load](load.md)**
), and is used to
...
The
*interp*
 argument is the path name of the interpreter from
which to unload the package (see the
**[interp](interp.md)**
 manual entry for
details); if
*interp*
...
**[load](load.md)**
 command, Tcl associates two reference counts to the library
...
a shared library can be loaded only once by Tcl (with the first
**[load](load.md)**

call on the file), these counters track how many interpreters use the library.
Each subsequent call to
**[load](load.md)**
 after the first simply increments the
...
The *interp* argument identifies the interpreter from which the library is to be unloaded. The unload procedure must return **[TCL_OK](catch.md)** or **[TCL_ERROR](catch.md)** to indicate whether or not it completed successfully; in the event of an error it should set the interpreter's result to point to an error message. In this case, the result of the **unload** command will be the result returned by the unload procedure.
...
The
**unload**
 command cannot unload libraries that are statically
...
**unload**
 on such a library (as the library will be completely detached
from the application while some interpreters will continue to use it).

### EXAMPLE

If an unloadable module in the file
**foobar.dll**
 had been loaded
using the
**[load](load.md)**
