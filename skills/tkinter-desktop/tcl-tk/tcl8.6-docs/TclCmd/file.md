...
*Name*
 is the name of a file; if it starts with a tilde, then tilde
substitution is done before executing the command (see the manual entry for

**[filename](filename.md)**
 for details).
*Option*
 indicates what to do with the
file name.  Any unique abbreviation for
*option*
 is acceptable.  The
valid options are:
...
is determined using the same rules as for **[string match](string.md)**.
...
subprocess via **[exec](exec.md)** under Windows (see **EXAMPLES** below).
**file normalize***name*
...
**Windows**
The **file owned** subcommand uses the user identifier (SID) of
