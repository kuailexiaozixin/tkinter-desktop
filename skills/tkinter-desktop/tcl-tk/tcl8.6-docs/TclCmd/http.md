...
It does not export the commands **cleanup**, **code**, **data**, **[error](error.md)**, **meta**, **ncode**, **size**, or **status**.

### DESCRIPTION

The
**http**
 package provides the client side of the HTTP/1.1
...
firewalls.  The package is compatible with the
**Safesock**
 security
...
pipelined.  See the **PERSISTENT SOCKETS** section for details. The default
is 1.
**-postfresh** *boolean*
Specifies whether requests that use the **POST** method will always use a
fresh socket, overriding the **-keepalive** option of
command **http::geturl**.  See the **PERSISTENT SOCKETS** section for
...
non-empty.
The **::http::geturl** command runs the **-proxyfilter** callback inside a **[catch](catch.md)** command. Therefore an error in the callback command does not call the **[bgerror](bgerror.md)** handler. See the **ERRORS** section for details.
...
request
will be automatically retried; if boolean **false** it will not, and the
application
that uses **http::geturl** is expected to seek user confirmation before
retrying the POST.  The value **true** should be used only under certain
conditions. See the **PERSISTENT SOCKETS** section for details. The
default is 0.
**-urlencoding** *encoding*
The *encoding* used for creating the x-url-encoded URLs with
**::http::formatQuery** and **::http::quoteString**.
The default is **utf-8**, as specified by RFC
...
**::http::formatQuery** or **::http::quoteString**
...
the version numbers of **http** and **[Tcl](Tcl.md)**, and is (for example)
“**Mozilla/5.0 (Windows; U; Windows NT 10.0) http/2.9.0 Tcl/8.6.9**”.
...
numbers of **http** and **[Tcl](Tcl.md)**.
**-zip** *boolean*
If the value is boolean **true**, then by default requests will send a header
“**Accept-Encoding: gzip,deflate,compress**”.
If the value is boolean **false**, then by default this header will not be
sent.  In either case the default can be overridden for an individual request by
supplying a custom **Accept-Encoding** header in the **-headers** option
of **http::geturl**. The default is 1.
...
this is auto-detected (anything not beginning with a **[text](../TkCmd/text.md)** content
...
The **::http::geturl** command runs the **-command** callback inside a **[catch](catch.md)** command. Therefore an error in the callback command does not call the **[bgerror](bgerror.md)** handler. See the **ERRORS** section for details.
...
The **::http::geturl** command runs the **-handler** callback inside a **[catch](catch.md)** command. Therefore an error in the callback command does not call the **[bgerror](bgerror.md)** handler. See the **ERRORS** section for details.

**-headers** *keyvaluelist*
This option is used to add headers not already specified
by **::http::config** to the HTTP request.  The
...
**-keepalive** *boolean*
If boolean **true**, attempt to keep the connection open for servicing
...
**-myaddr** *address*
Pass an specific local address to the underlying **[socket](socket.md)** call in case
...
**-query** *query*
This flag (if the value is non-empty) causes **::http::geturl** to do a
POST request that passes the string
*query* verbatim to the server as the request payload.
The content format (and encoding) of *query* is announced by the request
header **Content-Type** which is set by the option **-type**.  Any value
of **-type** is permitted, and it is the responsibility of the caller to
supply *query* in the correct format.
...
uses **[vwait](vwait.md)**.  Also, it is not useful for the case where
...
**::http::error** *token*
This is a convenience procedure that returns the **[error](error.md)** element
of the state array.
**::http::status** *token*
...
message for use by **http**.  Calling this command does not raise an
...
propagate to **http**.  The command allows **http** to provide a
precise error message rather than a general one.  The command returns the
value provided by the last call with argument *message*, or the empty
...
(via **::http::register**) if there was such a handler, and an error if
...
The
**::http::geturl**
 procedure will raise errors in the following cases:
...
The **::http::geturl** command runs the **-command**, **-handler**, and **-proxyfilter** callbacks inside a **[catch](catch.md)** command. Therefore an error in the callback command does not call the **[bgerror](bgerror.md)** handler. When debugging one of these callbacks, it may be convenient to report errors by using a **[catch](catch.md)** command within the callback command itself, e.g. to write an error message to stdout.
...
is raised, but the status of the transaction will be **[eof](eof.md)**.
**error**
The error message will also be stored in the **[error](error.md)** status
...
**binary**
This is boolean **true** if (after decoding any compression specified
by the
“Content-Encoding”
response header) the HTTP response is binary.  It is boolean **false**
if the HTTP response is text.
**body**
The contents of the URL.  This will be empty if the **-channel**
option has been specified.  This value is returned by the **::http::data**
command.
**charset**
...
**status**
See description in the chapter **ERRORS** above for a
list and description of **status**.
...
**Connection**
 and
**Upgrade**
 client headers inform the server
