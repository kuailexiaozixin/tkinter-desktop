...
The
**oo::define**
 command is used to control the configuration of classes,
and the
**oo::objdefine**
 command is used to control the configuration of
objects (including classes as instance objects), with the configuration being
applied to the entity named in the
*class*
 or the
*object*
 argument.
...
arguments from
*subcommand*
 onwards are made into a list and that list is
used as the
*defScript*
 argument.
...
The following commands are supported in the
*defScript*
 for

**oo::define**
, each of which may also be used in the
*subcommand*
 form:

**constructor***argList bodyScript*
This creates or updates the constructor for a class. The formal arguments to
the constructor (defined using the same format as for the Tcl **[proc](proc.md)**
...
being constructed. Within the constructor, the **[next](next.md)** command should be
...
should use the **[next](next.md)** command to call the superclasses' destructors. Note
...
to the currently-defined **[bgerror](bgerror.md)** handler.
...
method (defined using the same format as for the Tcl **[proc](proc.md)** command) will
...
and
“**oo::define***cls***[self](self.md)***subcommand ...*”
...
**[oo::object](object.md)**, and that the parent classes of **[oo::object](object.md)** and
**[oo::class](class.md)** may not be modified.
By default, this slot works by replacement.
**unexport***name*?*name ...*?
...
but instead just through the **[my](my.md)** command visible in each object's
...
as for the Tcl **[proc](proc.md)** command) will be *argList*, and the body of the
...
methods provided by those classes (though a **oo::objdefine forward**ed
...
just through the **[my](my.md)** command visible in the object's context) by the
...
Some of the configurable definitions of a class or object are
*slotted
definitions*
. This means that the configuration is implemented by a slot
...
Internally, slot objects also define a method
**--default-operation**
...
always be called from a stack frame created by a call to **oo::define** or
**oo::objdefine**.
...
fully-qualified forms (so they can be compared with **[string equals](string.md)**): that
could be done by forwarding to **[namespace which](namespace.md)** or similar.
