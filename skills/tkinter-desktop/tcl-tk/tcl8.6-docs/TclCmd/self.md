...
The
**self**
 command, which should only be used from within the context of a
...
takes an argument,
*subcommand*
, that tells it what sort of information is
actually desired; if omitted the result will be the same as if
**self
object**
 was invoked. The supported subcommands are:
...
reported by **[info object](info.md)** **call** for the current method (except that this
...
with **[next](next.md)**, and that if the method was defined on an object then this
will fail.
...
the **[next](next.md)** command will invoke an actual method implementation), this
