...
All of the legal *access* values above may have the character **b** added as the second or third character in the value to indicate that the opened channel should be configured as if with the **[fconfigure](fconfigure.md)** **-translation binary** option, making the channel suitable for reading or writing of binary data.
...
(it is better to use the **[fconfigure](fconfigure.md)** command to put a file
in nonblocking mode).
...
*fileName*
 are treated as a list of arguments
that describe a command pipeline to invoke, in the same style as the
arguments for
**[exec](exec.md)**
.
In this case, the channel identifier returned by
**open**
 may be used
...
*access*
.
If write-only access is used (e.g.
*access*
 is
“
**w**
...
If read-only access is used (e.g.
*access*
 is
“
**r**
...
The id of the spawned process is accessible through the
**[pid](pid.md)**

command, using the channel id returned by
**open**
 as argument.

If the command (or one of the commands) executed in the command pipeline returns an error (according to the definition in **[exec](exec.md)**), a Tcl error is generated when **[close](close.md)** is called on the channel unless the pipeline is in non-blocking mode then no exit status is returned (a silent **[close](close.md)** with -blocking 0).

It is often useful to use the **[fileevent](fileevent.md)** command with pipelines so other processing may happen at the same time as running the command in the background.

### SERIAL COMMUNICATIONS

If
*fileName*
 refers to a serial port, then the specified serial port
is opened and initialized in a platform-dependent manner.  Acceptable
values for the
*fileName*
 to use to open a serial port are described in
the PORTABILITY ISSUES section.

The **[fconfigure](fconfigure.md)** command can be used to query and set additional configuration options specific to serial ports (where supported):
...
**m**, **s**; respectively signifying the parity options of
“none”,
“odd”,
“even”,
“mark”,
...
**-queue**
(Windows and Unix). The **-queue** option can only be queried.
...
The *signal* names are case-independent.
**{RTS 1 DTR 0}** sets the RTS output to high and the DTR output to low.
The BREAK condition (see below) is enabled and disabled with **{BREAK 1}** and
**{BREAK 0}** respectively.
It is not a good idea to change the **RTS** (or **DTR**) signal
with active hardware handshake **rtscts** (or **dtrdsr**).
...
e.g. **{CTS 1 DSR 0 RING 1 DCD 0}**.
The *signal* names are returned upper case.
...
In case of a serial communication error, **[read](read.md)** or **[puts](puts.md)**
returns a general Tcl file I/O error.
**[fconfigure](fconfigure.md)** **-lasterror** can be called to get a list of error details.
...
lines and handshaking. Here we are using the terms
*workstation*
 for
your computer and
*modem*
 for the external device, because some signal
...
**RTS**(output)
**Request To Send:** This hardware handshake line informs the modem that
...
**DSR**(input)
**Data Set Ready:** The complement to DTR. Tells the workstation that the
modem is ready to establish a link.
**DCD**(input)
**Data Carrier Detect:** This line becomes active when a modem detects a
“Carrier”
signal.
**RI**(input)
...
**[catch](catch.md)**
 serial read operations.  In cases of an error Tcl returns a
general file I/O error.  Then
...
it or your system is overloaded. Use **[fconfigure](fconfigure.md)** **-sysbuffer** to avoid a
...
Wrong parity settings with **[fconfigure](fconfigure.md)** **-mode** or a noisy data line (RXD)
may cause this error.
**FRAME**
A stop-bit error has been detected by your UART.
Wrong mode settings with **[fconfigure](fconfigure.md)** **-mode** or a noisy data line (RXD)
may cause this error.
**BREAK**
...
**com***X*****, where *X* is a number, generally from 1 to 9.
A legacy form accepted as well is **com***X***:**. This notation only
...
Files opened in the “**a**” mode or with the **[APPEND](append.md)** flag set are implemented by seeking immediately before each write, which is not an atomic operation and does not carry the guarantee of strict appending that is present on POSIX platforms.

**Unix**
Valid values for *fileName* to open a serial port are generally of the
form **/dev/tty***X*, where *X* is **a** or **b**, but the name
...
See the **PORTABILITY ISSUES** section of the **[exec](exec.md)** command for additional information not specific to command pipelines about executing applications on the various platforms
