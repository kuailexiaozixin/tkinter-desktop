...
*body*
 argument by passing it recursively
...
matches
*string*
 and no default is given, then the
**switch**
...
(i.e. the same as implemented by the **[string match](string.md)** command).
**-regexp**
When matching *string* to the patterns, use regular
expression matching
(as described in the **[re_syntax](re_syntax.md)** reference page).
**-nocase**
...
string (i.e. the *string* argument to **switch**) matched, the
...
on.  When a **default** branch is taken, the variable will have the
empty list written to it.  This option may be specified at the same
time as the **-indexvar** option.
**-indexvar** *varName*
This option (only legal when **-regexp** is also specified)
...
string (i.e. the *string* argument to **switch**) matched, in a
similar way to the **-indices** option to the **[regexp](regexp.md)** can
...
The
**switch**
 command can match against variables and not just
literals, as shown here (the result is
*2*
):
