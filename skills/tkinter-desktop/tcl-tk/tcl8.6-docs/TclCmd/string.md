...
Concatenate the given *string*s just like placing them directly
next to each other and return the resulting compound string.  If no
*string*s are present, the result is an empty string.
This primitive is occasionally handier than juxtaposition of strings when mixed quoting is wanted, or when the aim is to return the result of a concatenation without resorting to **[return](return.md)** **-level 0**, and is more efficient than building a list of arguments and using **[join](join.md)** with an empty join string.
...
forms described in **STRING INDICES**), then the search is
...
A *charIndex* of 0 corresponds to the first character of the
string.  *charIndex* may be specified as described in the
**STRING INDICES** section.
...
the value, 0 is returned and the *varname* will contain -1.
**list**
Any proper list structure, with optional surrounding whitespace. In
case of improper list structure, 0 is returned and the *varname*
...
of the forms described in **STRING INDICES**), then only the
...
as in the form returned by **[array get](array.md)**.  Each instance of a
...
and
“a”),
with **-nocase** this is considered like {[A-Za-z]} (and
...
**STRING INDICES**). An index of **0** refers to the first
character of the string; an index of **end** refers to last
...
*count* times. If *count* is 0, the empty string will be
returned.
**string replace***string first last* ?*newstring*?
Removes a range of consecutive characters from *string*, starting
with the character whose index is *first* and ending with the
character whose index is *last* (using the forms described in
**STRING INDICES**).  An index of 0 refers to the
...
then it is treated as if it were **end**.  The initial string is
returned untouched, if *first* is greater than *last*, or if
*first* is equal to or greater than the length of the initial string,
or *last* is less than 0.
...
modifying.  If *last* is not specified, only the character at position
*first* is converted, otherwise *last* refers to the char index
in the string to stop at (inclusive).  *first* and *last* may be
...
*last* is not specified, only the character at position *first*
is converted, otherwise *last* refers to the char index in
...
*string* in memory when encoded as Tcl's internal modified UTF-8;
Tcl may use other encodings for *string* as well, and does not
guarantee to only use a single encoding for a particular *string*.
...
*Compatibility note:* it is likely that this subcommand will be withdrawn in a future version of Tcl. It is better to use the **[encoding convertto](encoding.md)** command to convert a string to a known encoding and then apply **string length** to that.
...
*charIndex* of *string*.  *charIndex* may be specified using the
forms in **STRING INDICES**.  A word is considered to be any contiguous
...
When referring to indices into a string (e.g., for
**string index**
...
the char specified at this integral index (e.g., **2** would refer to the
“c”
in
“abcd”).
**end**
