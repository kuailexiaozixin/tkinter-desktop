...
The
**registry**
 package provides a general set of operations for
manipulating the Windows registry.  The package implements the

**registry**
 Tcl command.  This command is only supported on the
...
using the same rules as for **[string match](string.md)**.  If the
...
names matching *pattern* are returned.  Matching is determined
using the same rules as for **[string match](string.md)**.
...
types, the
**registry**
 command returns the data in a different form to
