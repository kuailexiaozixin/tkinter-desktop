...
Database Name: **[style](ttk_style.md)**
Database Class: **[Style](ttk_style.md)**
...
Typically the **-xscrollcommand** option consists of the path name of a **[scrollbar](scrollbar.md)** widget followed by “set”, e.g. “.x.scrollbar set”. This will cause the scrollbar to be updated whenever the view in the window changes.
...
"Elements" section of *ttk::intro(n)* explain which value is actually
used.
...
Database Name: **[font](font.md)**
Database Class: **[Font](font.md)**
...
The rest of the list is a sequence of *statespec / value* pairs
as per **[style map](ttk_style.md)**, specifying different images to use when
...
by **-textvariable** for the widgets supporting this option).
...
Some widgets accept other **identify** subcommands described
...
Adjusts the view in the window so that the item *fraction* of the
...
*number* average-width characters on the display;  if it is
**pages** then the view adjusts by *number* screenfuls.
If *number* is negative then items farther to the top
...
The **background** state is set for widgets in a background window,
...
This is similar to the **active** state;
