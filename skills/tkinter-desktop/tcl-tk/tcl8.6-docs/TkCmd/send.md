...
This command arranges for
*cmd*
 (and
*arg*
s) to be executed in the
application named by
*app*
.  It returns the result or
error from that command execution.

*App*
 may be the name of any application whose main window is
...
more
*arg*
s are present, they are concatenated to form the
command to be executed, just as for the
**[eval](../TclCmd/eval.md)**
 command.
...
If the
**send**
 command is removed from an application (e.g.
with the command
**[rename](../TclCmd/rename.md)**
**send {}**
) then the application
will not respond to incoming send requests anymore,  nor will it
be able to issue outgoing requests.
Communication can be reenabled by invoking the
**[tk appname](tk.md)**
...
and rejects incoming sends unless (a)
**xhost**
...
such as that provide by
**xauth**
.
Under Windows,
**send**
 is currently disabled.  Most of the
