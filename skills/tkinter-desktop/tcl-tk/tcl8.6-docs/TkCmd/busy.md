...
The
**tk busy**
 command provides a simple means to block mouse pointer events
...
Often, the **tk busy** command can be used instead of Tk's **[grab](grab.md)** command. Unlike **[grab](grab.md)** which restricts all user interactions to one widget, with the **tk busy** command you can have more than one widget active (for example, a “Cancel” dialog and a “Help” button).
...
All the widgets within **.top** (including **.top**) are now busy. Using **[update](../TclCmd/update.md)** insures that **tk busy** command will take effect before any other user events can occur.
...
this case the command returns the empty string. *Option* may have any of
the values accepted by the **hold** operation.
Please note that the option database is referenced through *window*. For example, if the widget **.frame** is to be made busy, the busy cursor can be specified for it by either **[option](option.md)** command:
...
*Window* must be the name of a widget specified in the **hold**
...
hierarchy) appear busy. *Window* must be a valid path name of a Tk widget.
...
**[update](../TclCmd/update.md)** should be called immediately afterward to insure that the hold
...
Returns the status of a widget *window*. If *window* presently can not
...
Note that the
**tk busy**
 command does not currently have any effect on macOS
