...
command in the
**File**
 menu. Its purpose is for the user to select an
existing file
*only*
. If the user enters a non-existent file, the
...
new files, it should do so by providing a separate
**New**
 menu command.
...
If a **[File types](../TclCmd/file.md)** listbox exists in the file dialog on the particular
...
empty list, or if the **[File types](../TclCmd/file.md)** listbox is not supported by the
...
*typeName*
 is the name of the file type described by this
file pattern and is the text string that appears in the
**[File types](../TclCmd/file.md)**
...
*macType*
s is optional and may be omitted for applications that do
